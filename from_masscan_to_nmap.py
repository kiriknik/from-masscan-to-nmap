...
def xmlDifference(xmlFile1,xmlFile2):
    ...
    for element in wtf1:
        if wtf2.get(element)!= wtf1.get(element):
            print "On the first scan on "+str(element)+ " next ports:" +str (wtf1.get(element))+ ". On the second scan next ports " + str(wtf2.get(element))
            print
            #print (str(element)+" unidentically")
        else:
            pass
    for element in wtf2:
        if wtf1.get(element)!= wtf2.get(element):
            print "On the first scan on " + str(element) + " next ports:" + str(
                wtf1.get(element)) + ". On the second scan next ports:" + str(wtf2.get(element))
            print
            #print (str(element)+" unidentically")
        else:
            pass
            #print (str(element)+" identically")
        ...
        if options['no_printer_scan']:
            ...
            if flag!=0:
                ...
                if options['add_auth_and_safe_scripts']:
                    string= str("sc "+element+" "+element+" -p"+str(wtf[element]).replace("['","").replace("']","").replace("'","").replace(" ",""))
                else:
